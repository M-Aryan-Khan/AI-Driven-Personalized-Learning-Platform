...
export default function ResetPasswordPage() {
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === "Backspace" && !formData.resetCode[index] && index > 0 && inputRefs.current[index - 1]) {
      inputRefs.current[index - 1].focus()
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        router.push(`/auth/login?reset=success&email=${encodeURIComponent(email)}`)
      } catch (error: any) {
      ...
      <div className="flex-1 flex items-center justify-center p-4">
        ...
        >
          <div className="text-center space-y-2">
            <h1 className="text-2xl font-bold text-deep-cocoa">Reset your password</h1>

            <p className="text-rose-dust">
              Enter the 6-digit code sent to <span className="font-medium text-deep-cocoa">{email}</span> and create a
              new password
            </p>
          </div>
