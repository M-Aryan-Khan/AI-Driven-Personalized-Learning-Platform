...
>(({ className, ...props }, ref) => {
  // Ensure children is a string to prevent React errors
  const safeProps = { ...props }
  if (safeProps.children && typeof safeProps.children === "object") {
    try {
      safeProps.children = JSON.stringify(safeProps.children)
    } catch (e) {
      safeProps.children = String(safeProps.children)
    }
  }

  return <ToastPrimitives.Description ref={ref} className={cn("text-sm opacity-90", className)} {...safeProps} />
})
